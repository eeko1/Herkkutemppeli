...
interface Product {
  ...
  product_allergens: string;
  product_price: number;
  product_category_id?: string; // Add this property to your product type
}
...
function checkButtons() {
  ...
  addButtons.forEach((button) => {
    ...
    const productPrice = (productElement
      ?.querySelector(".productPrice") as HTMLElement)?.innerText;
    const productName = (productElement
      ?.querySelector(".productName") as HTMLElement)?.innerText;
    const productImage = (productElement
      ?.querySelector(".productImage") as HTMLImageElement)?.src;
