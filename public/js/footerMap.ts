...
// Assuming L is from 'leaflet', and you have the types installed from '@types/leaflet'
const map: L.Map = L.map('map').setView([60.1699, 24.938], 13);

L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
  maxZoom: 19,
}).addTo(map);

const restaurantIcon: L.Icon = L.icon({
...
restaurants.forEach((restaurant: Restaurant) => {
  const restaurantMarker: L.Marker = L.marker([
