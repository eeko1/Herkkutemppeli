...
let totalCost = 0;
let cartIds = [];
const elements = shoppingCartDialog.querySelectorAll('a, button, input, textarea, select, details, [tabindex]:not([tabindex="-1"])');
...
function searchItemId() {
    ...
    items?.forEach((item) => {
        ...
        let matchingItems = allProducts.filter((product) => product.product_name === itemName);
...
function addToCart(productName, productPrice, productImage) {
    ...
    for (const item of existingItems || []) {
        ...
        if (existingItemName == productName) {
            const quantitySpan = item.closest(".quantity-controls")?.querySelector(".quantity");
    ...
    decreaseButton.addEventListener("click", () => {
        ...
        const quantitySpan = decreaseButton.closest(".quantity-controls")?.querySelector(".quantity");
        let quantity = parseInt(quantitySpan?.textContent ?? "0", 10);
        if (quantity > 0) {
            quantity--;
            (quantitySpan ?? document.createElement('span')).textContent = quantity.toString();
    ...
    increaseButton.addEventListener("click", () => {
        ...
        const quantitySpan = increaseButton.closest(".quantity-controls")?.querySelector(".quantity");
        let quantity = parseInt(quantitySpan?.textContent ?? "0", 10);
        quantity++;
        totalCost += parseFloat(productPrice);
        updateTotalCost(totalCost);
